...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    return {
      title: `${artist.name} - Duy Vu Ink`,
      description: `Explore tattoos by ${artist.name} at Duy Vu Ink.`,
    };
  } catch {
    return {
      title: "Error",
...
export default async function ArtistDetailPage({ params }: Props) {
  const { slug } = await params;
  const [artist, tattoos] = await Promise.all([getArtistBySlug(slug), getTattoosByArtist(slug)]);
