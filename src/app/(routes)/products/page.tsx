...
import { HeaderPage, PaginationItem, ProductList, ProductToolbar } from "@/components";
import { getBrands, getCategories, getProducts } from "@/lib/api/products";

export const metadata: Metadata = {
  title: "Products - Duy Vu Ink",
  description:
...
export default async function ProductsPage({ searchParams }: Props) {
  ...
  const [productsData, brands, categories] = await Promise.all([getProducts(params), getBrands(), getCategories()]);
  ...
  return (
    <main className="max-w-screen-xl mx-auto my-16 px-4 xl:px-0">
      <HeaderPage title="Our Products" subtitle="Discover our selection of aftercare products and tattoo supplies." />
      <>
        ...
        {products.length === 0 ? (
          <section className=" relative flex justify-center items-center w-full h-[400px] rounded-none overflow-hidden">
            <div className="z-10 text-center text-gray-500">
              No products found for <span className="ml-2 font-semibold text-primary">{search?.toUpperCase()}</span>
            </div>
